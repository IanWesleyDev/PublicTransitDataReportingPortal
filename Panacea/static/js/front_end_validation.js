...
let error_dict = {};  //dictionary for storing errors
...
//List of  fields to apply total greater than to
...
function getNextElement(elem, className) {
    let next = elem.nextElementSibling; // set next element to "nextElementSibling" relative to passed element.
...
function getPreviousElement(elem, className) {
    let next = elem.previousElementSibling; // set next element to "nextElementSibling" relative to passed element.
    while (next && !next.classList.contains(className)) { // check for existence and class
        next = next.previousElementSibling; // if it exists, but the class does not, move to the next element and repeat.
    }
    return next; // return whatever was found, or null
}

//Reads data from forms and stores in an object to tell if values change
...
function remove_err_msg(input_elem, err_msg) {
    ...
    if (element_has_error(input_elem) && err_msg == null){
        ...
        if(!element_has_error(input_elem)){
            input_elem.classList.remove("is-invalid");
            delete error_dict[input_elem.name];
        }
    }
}
